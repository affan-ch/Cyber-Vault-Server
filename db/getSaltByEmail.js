...
async function getSaltByEmail(link, databaseName, collectionName, object) {
    ...
    try {
        ...
        const result = await collection.findOne({ email: object.email});

        if (result) {
            // If a matching record is found, return the name and email
            return { salt: result.salt };
        } else {
